...
from ansiblemddoc.AutoDocumenter import Writer
...
class AnsibleMdDoc:
    ...
    def _cli_args(self):
        ...
        :return: args objec
        """
        usage = '''ansible-mddoc [project_directory] [options]'''
        parser = argparse.ArgumentParser(description='Generate documentation from roles and playbooks', usage=usage)
        ...
        parser.add_argument('--main-file', action='store', dest="playbook_main", help="specifies the main file (useful for documenting a playbook)."
                                                                     "If not specified tasks/main.yml will be used")
    ...
    def _parse_args(self,args):
        ...
        # playbook_main 
        if args.playbook_main is not None:
            main_file_path = os.path.join(self.config.get_base_dir(), args.playbook_main)
            if os.path.isfile(main_file_path):
                self.config.playbook_main = args.playbook_main
                self.log.info("Using playbook main file: "+main_file_path)
            else:
                self.log.warn("Main file not found: "+main_file_path)
                sys.exit()
